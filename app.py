...
def main():
    ...
    There is a checkbox on the side that enables the display of causal relationships(currently for English language only). This feature displays the extracted causal structures of the narrative, generated by GPT and other large language models (LLMs). The causal relationships are extracted by leveraging large language models (LLMs) to construct <u>causal graphs</u>, where events are represented as nodes and causal links as directed edges. Using few-shot prompting, LLMs are guided with example demonstrations to identify and connect causal events in a story. These extracted causal structures enhance tasks like story quality evaluation and video-text alignment, showcasing the practical utility of event causality in computational storytelling. For more, refer to the [Event Causality Is Key to Computational Story Understanding](https://arxiv.org/abs/2311.09648) paper.
    
    The graph shown in the causal relationship display highlights immediate predecessors in <span style="color: #66ff66; background-color: #a1a1a1; padding: 2px 2px; border-radius: 4px;"><strong>green</strong></span> and immediate successors in <span style="color: #4d50fa; background-color: #a1a1a1; padding: 2px 2px; border-radius: 4px;"><strong>blue</strong></span>. The transcript sentences are color-coded to match the corresponding nodes in the causal graph.
