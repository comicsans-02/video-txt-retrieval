...
def generate_transcript_html(transcript_data):
    ...
    for index, item in enumerate(transcript_data):
        ...
        text = item['text']
        matched = item.get('matched', 'no').lower().strip()  
...
def main():
    ...
    This platform serves as a demonstration of the research presented in [Multilingual Synopses of Movie Narratives: A Dataset for Vision-Language Story Understanding](https://arxiv.org/abs/2406.13092). At any given timestamp in the selected video, the highlighted text (in <span style="color: #ffff66; background-color: #a1a1a1; padding: 2px 2px; border-radius: 4px;"><strong>yellow</strong></span>) corresponds to the video clip segments that best aligns with that moment. The user may also manually select a sentence to seek to the corresponding aligned video segment.

    ### **Causal Relationships**
    There is a checkbox on the side that enables the display of causal relationships(currently for English language only). This feature displays the extracted causal structures of the narrative, generated by GPT and other large language models (LLMs). The graph shown in the causal relationship display highlights immediate predecessors in <span style="color: #66ff66; background-color: #a1a1a1; padding: 2px 2px; border-radius: 4px;"><strong>green</strong></span> and immediate successors in <span style="color: #4d50fa; background-color: #a1a1a1; padding: 2px 2px; border-radius: 4px;"><strong>blue</strong></span>. The transcript sentences are color-coded to match the corresponding nodes in the causal graph.

    The causal relationships are extracted by leveraging large language models (LLMs) to construct <u>causal graphs</u>, where events are represented as nodes and causal links as directed edges. Using few-shot prompting, LLMs are guided with example demonstrations to identify and connect causal events in a story. These extracted causal structures enhance tasks like story quality evaluation and video-text alignment, showcasing the practical utility of event causality in computational storytelling. For more, refer to the [Event Causality Is Key to Computational Story Understanding](https://arxiv.org/abs/2311.09648) paper.
    ...
    videos = {
        ...
        'Portuguese': ['nuks1jnUbss.mp4', 'SpmYKDC69Ms.mp4'],  
    ...
    if selected_language == 'English':
        ...
        st.sidebar.write("""
        This checkbox enables the display of causal relationships extracted from the narrative.
        """)
        st.session_state.use_causal_relations = use_causal_relations  
    else:
    ...
    <style>
        ...
        #graph {{
            ...
            flex-direction: column;
            align-items: center;
            gap: 20px; /* Increase spacing between node groups */
        }}
        .node-group {{
            display: flex;
            flex-direction: column;
        ...
        .node-container {{
            ...
            flex-direction: row;
            align-items: center;
            gap: 30px; /* Increase spacing between nodes */
            flex-wrap: wrap;
    ...
    # Display the references in BibTeX format
    st.markdown("""
    ### References
    ```bibtex
